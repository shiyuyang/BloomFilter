...
void BloomFilter::Init(std::vector<uint64_t> bits, uint32_t hash_times)
{
	_hash_times = hash_times;
	_bit_sets.clear();
	for (auto it = bits.begin(); it != bits.end(); it++)
	{
		std::bitset<64> bit_set(*it);
...
bool BloomFilter::IsFull(){
    uint32_t count = 0;
    uint32_t half_bits = ONE_BYTE * BIT_WIDTH * _bit_sets.size() / 2;
    for(auto it = _bit_sets.begin();it != _bit_sets.end();it++){
        count += it->count();
        if(count >= half_bits){
            return true;
...
std::vector<uint64_t> BloomFilter::SerializeFilter()
{
	...
	for (auto it = _bit_sets.begin(); it != _bit_sets.end(); it++)
	{
		serialized_vec.push_back(it->to_ulong());
	}
...
{
	uint32_t pos = 0;
	for (uint32_t i = 0; i < _hash_times; i++)
	{
